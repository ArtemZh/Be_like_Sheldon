...
Стан на 2026-08-31. Гілка `feat/daytrip-isochrones`, 36 комітів; робота над
трьома режимами і скрінсейвер для macOS ще не закомічені (`git status`:
`macos/`, `web/src/frames.*`, `web/src/mirror.*`, зміни в `map.js`, `flap.js`,
`style.css`, `README.md`, `.gitignore`).
...
cd web && npm test            # 117 тестів фронтенду
...
Той самий сайт на весь екран після простою. Складається з трьох частин:

| Файл | Що робить |
|---|---|
| `Site.h/.m` | віддає сайт із бандла через власну схему `sheldon://` |
| `Companion.m` | застосунок `Be like Sheldon Screen.app` — вікно на кожен екран, вихід від руху миші |
| `idle-watch.sh` + `install-agent.sh` | агент launchd: раз на 30 с питає простій і вмикає застосунок |
| `SheldonSaver.m` | тонкий `.saver`: налаштування й спроба запустити застосунок |

```bash
macos/build.sh              # сайт → застосунок → .saver
macos/install-agent.sh 300  # вмикати після 5 хвилин простою
open "macos/build/Be like Sheldon Screen.app"   # перевірити руками
```

**Чому не звичайна заставка.** Спершу все жило всередині `.saver`, і воно
майже запрацювало — але не до кінця. Перелік того, що довелося зробити, аби
дійти хоча б до «майже», варто прочитати, перш ніж повертатись до цієї ідеї:

- Рушій за сеанс кілька разів створює новий в'ю. Наївний код щоразу вантажив
  адресу наново (38 завантажень за сеанс), і застосунок ніколи не встигав
  ініціалізуватись. Лікується одним веб-в'ю на екран у статиці.
- Процес присипляє App Nap за пʼять секунд — рятує `beginActivityWithOptions`.
- `evaluateJavaScript` у цьому середовищі вішає процес намертво: з веб-в'ю
  можна говорити лише в один бік, через `WKScriptMessageHandler`.
- WebKit вважає сторінку прихованою й глушить `requestAnimationFrame`, а
  MapLibre саме на ньому застосовує стиль — без підміни `rAF` таймером карти
  немає взагалі.
- Разом зі сторінкою присипляють і воркери; допомагає беззвучний
  аудіоконтекст, який тримає сторінку «звучною».
- Шар WebGL на екран не потрапляє, тож кадр карти доводиться копіювати в
  2D-полотно (`mirror.js`), і саме **попередній** кадр: у щойно замовленому
  верхні шари ще не лягли.

Після всього цього сторінка працює бездоганно (журнал це показує), але
**картинка застигає**: macOS показує шари веб-в'ю у вікні заставки один раз і
далі оновлює їх коли захоче. Поштовхи композитора (зміна розкладки) дають
оновлення раз на пів секунди й теж ненадійно; знімки `WKSnapshot` і власне
вікно поверх дають чорний екран. Останнє: поверх працюючої системної заставки
macOS взагалі не показує вікна інших застосунків — тому `.saver` не може
показати й компаньйона.

Звідси теперішня схема: карту показує звичайний застосунок, а простій ловить
агент. У застосунку композиція така сама, як у браузері, і все працює.
...
`index.html` ще до першого кадру — інакше блимає денний вигляд. `frames.js`
веде кадри таймером, коли `document.hidden`, а `reportToSaver` коротко
звітує про стан у журнал (`WKScriptMessageHandler`) — у скрінсейвері це був
єдиний спосіб хоч щось побачити.
...
- **Скрінсейвер малює карту через дзеркало**, тобто зайвий раз копіює кадр на
  процесорі. На 4К це помітно; якщо колись знайдеться спосіб переконати
  WebKit, що сторінка видима, дзеркало й підміну `rAF` можна буде викинути.
